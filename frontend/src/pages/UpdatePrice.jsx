...
export default function UpdatePrice() {
  ...
  const [search, setSearch] = useState(() => location.state?.prefillName || '');
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!canUpdate) {
      setError('Only managers or admins can change prices.');
      return;
    }
    if (!selected) {
    ...
    setMessage('');
    try {
      const res = await fetch(`${API_BASE}/api/products/${selected.id}/price`, {
        method: 'PUT',
        headers: {
      ...
      setMessage('Price updated and logged successfully.');
    } catch (err) {
        ...
        <main className="app-content">
          <div className="update-card">
            <div className="update-card__header">
              <div>
                <p className="pill pill-info">Price only</p>
                <h1>Update Price</h1>
                <p className="muted">Search by product name, pick an item, then set its price. Action is logged.</p>
              </div>
            ...
            {!canUpdate && (
              <div className="error-box">
                Only managers or admins can change prices.
              </div>
            )}
